...
class DesignationsController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->Designations->save($designation)) {
                $this->Flash->success(__('The {0} has been saved.', 'Designation'));

                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error(__('The {0} could not be saved. Please, try again.', 'Designation'));
        }
    ...
    public function delete($id = null)
    {
        ...
        if ($this->Designations->delete($designation)) {
            $this->Flash->success(__('The {0} has been deleted.', 'Designation'));
        } else {
            $this->Flash->error(__('The {0} could not be deleted. Please, try again.', 'Designation'));
        }
