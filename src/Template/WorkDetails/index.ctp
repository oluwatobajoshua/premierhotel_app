            ...
            <tbody>
              <?php foreach ($workDetails as $workDetail): ?>
                <tr>
                  ...
                  <td><?= $workDetail->has('employee') ? $this->Html->link($workDetail->employee->name_desc, ['controller' => 'Employees', 'action' => 'view', $workDetail->employee->id]) : '' ?></td>
                  ...
                  <td class="actions text-right">
                      <?= $this->Html->link(__('View'), ['action' => 'view', $workDetail->id], ['class'=>'btn btn-info btn-xs']) ?>
                      <?= $this->Html->link(__('Edit'), ['action' => 'edit', $workDetail->id], ['class'=>'btn btn-warning btn-xs']) ?>
                      <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $workDetail->id], ['confirm' => __('Are you sure you want to delete # {0}?', $workDetail->id), 'class'=>'btn btn-danger btn-xs']) ?>
                  </td>
                </tr>
              <?php endforeach; ?>
            </tbody>
