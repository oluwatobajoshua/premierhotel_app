    ...
    <div class="box-header with-border">
        ...
        <form action="<?php echo $this->Url->build(); ?>" method="POST" class='pull-right' id='empForm'>
            <div class="col-md-3" style="width:400px">
                ...
                <?php echo $this->Form->select('cadre',$cadreList,['empty' => "Select Cadre"]);?>
                <br>
                <label class="form-check-label" for="">Bank</label>
                <?php echo $this->Form->select('bank',$bankList,['empty' => "Select Bank", 'rel'=>'tooltip', 'onChange'=>'document.getElementById("empForm").submit();']);?>
                <a href="" class="btn btn-xs btn-default no-print" onClick="window.print();"><i class="fa fa-print"></i>
                    Print</a>
            </div>
        </form>
    </div>
